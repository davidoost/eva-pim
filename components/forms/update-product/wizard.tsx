...
type ProductForEdit = SelectProduct;

interface UpdateProductWizardProps {
  namespace: string;
  taxCodes: TaxCode[];
...
function deriveInitialVariantValues(
  children: ProductForEdit["children"],
): string[] {
...
function deriveInitialSecondaryVariantValues(
  children: ProductForEdit["children"],
): string[] {
  const firstChild = children[0];
  if (!firstChild) return [""];
  const values = firstChild.children
    .map((gc) => gc.variationValue ?? "")
    .filter(Boolean);
  return values.length > 0 ? values : [""];
}

export default function UpdateProductWizard({
  namespace,
  taxCodes,
  product,
}: UpdateProductWizardProps) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    imageUrls: [],
    removedImagePaths: [],
    hasVariants: product.children.length > 0,
    variationProperty: product.childrenVariationProperty ?? "",
    variantValues: deriveInitialVariantValues(product.children),
    hasSecondaryVariant: product.children.some((c) => c.children.length > 0),
    secondaryVariationProperty:
      product.children[0]?.childrenVariationProperty ?? "",
    secondaryVariantValues: deriveInitialSecondaryVariantValues(
      product.children,
    ),
  });
  ...
  async function handleNext() {
    ...
    if (step === 1) {
      ...
      if (result?.removedPaths) update("removedImagePaths", result.removedPaths);
    }
    setStep((s) => s + 1);
  }
  ...
  function handleSubmit() {
    ...
    const validSecondaryValues = form.secondaryVariantValues.filter((v) =>
      v.trim(),
    );
    startTransition(async () => {
      const result = await updateProductAction(namespace, product.id, {
        ...
        removedImagePaths: form.removedImagePaths,
        variants:
        ...
        secondaryVariants:
          ...
          validSecondaryValues.length > 0
            ? {
                property: form.secondaryVariationProperty.trim(),
            ...
            <div className="flex flex-col gap-3">
              <TextField variant="secondary">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    update("variationProperty", e.target.value)
                  }
              ...
              <Switch
                ...
                onChange={(val: boolean) =>
                  update("hasSecondaryVariant", val)
                }
              >
                {() => (
                  <>
                ...
                <>
                  <TextField variant="secondary">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        update("secondaryVariationProperty", e.target.value)
                      }
                    ...
                    <Description className="text-xs text-muted">
                      Applied under each{" "}
                      {form.variationProperty || "primary"} variant.
                    </Description>
                  </TextField>
                  ...
                  <div className="flex flex-col gap-2">
                          ...
                          <Button
                            ...
                            onPress={() =>
                              update(
                                ...
                                form.secondaryVariantValues.filter(
                                  (_, j) => j !== i,
                                ),
                              )
                            }
                    ...
                    <Button
                      ...
                      className="self-start"
                      onPress={() =>
                        update("secondaryVariantValues", [
                          ...form.secondaryVariantValues,
                          "",
                        ])
                      }
                    >
